"""
Gradio API Proxy for Kani TTS 2 (Multilingual / kani-pt)

Translates our REST API into Gradio calls against the HuggingFace Space.
Supports voice cloning via the session-based embedding flow:
  1. Upload audio to /generate_embedding_gpu  (stores embedding in Gradio State)
  2. Call /generate_speech_gpu with mode="generate" (uses stored embedding)
"""
...
app = FastAPI(title="Kani TTS Proxy – Multilingual")
...
# Session-level cloned voices: name -> True (embedding lives in Gradio State)
    ...
    return {
        ...
        "custom_params": {
            ...
            "model": {
                "type": "select",
                "options": MODELS,
                "default": MODELS[0] if MODELS else "KaniTTS-2-pt",
            },
        },
    }
...
async def synthesize(request: SynthesizeRequest):
    try:
        ...
        model_choice = custom.get(
            "model", MODELS[0] if MODELS else "KaniTTS-2-pt"
        )

        # Ensure we have speaker list (Gradio validates dropdown even in json mode)
        if not SPEAKERS:
            await refresh_api_metadata(force=True)
        default_speaker = SPEAKERS[0] if SPEAKERS else "Kore (en)"
...
async def clone_voice(
    ...
    Flow:
      1. Read & validate audio (pad if too short for the embedding model)
      2. Call /generate_embedding_gpu on the Gradio Space
      3. The embedding is stored in the Gradio session State
      4. Future calls with mode="generate" will use this embedding
    """
    try:
        client = await get_gradio_client()
        ...
        # Resample to 16 kHz if needed (simple linear interp)
        if sr != 16000:
        ...
        # Pad short audio to avoid conv kernel error
        ...
        # Write to temp file and upload via handle_file
